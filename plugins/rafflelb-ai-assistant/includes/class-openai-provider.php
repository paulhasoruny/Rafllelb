    ...
    public function chat($instructions, array $messages, Tools $tools, $safety_identifier = '') {
        ...
        for ($round = 0; $round <= self::MAX_TOOL_CALLS; $round++) {
            ...
            if (!$calls) {
                ...
                return $text !== '' ? $text : new \WP_Error('rafflelb_ai_empty_response', 'The AI provider returned no usable text.');
            }

            if ($tool_calls + count($calls) > self::MAX_TOOL_CALLS) {
                return new \WP_Error('rafflelb_ai_tool_limit', 'The read-only tool-call limit was reached.');
            }
