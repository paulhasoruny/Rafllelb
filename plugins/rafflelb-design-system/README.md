...
## 0.1.3 — Selection typography contract

- Keeps RaffleLB Design System as the only Manrope/font loader.
- Standardizes Selection Engine, public result/winner records, Selection-related product entry panels, and Selection Status controls on `--rl-font`.
- Removes the visual effect of the remaining handwritten Selection Engine font override by overriding font-family only.
- Adds future-facing support for components using the `rl-selection-*` naming convention.
- Does not change font sizes, weights, line heights, spacing, colors, layout, Selection logic, raffle logic, or WooCommerce behavior.
